                        ...
                        <form action="{{ route($form_action) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="card-body">
                                <div class="row">
                                            ...
                                            <select class="form-control custom-select-icon @error('role_id') is-invalid @enderror" name="role_id" id="role_id" onchange="AddBank();">
                                                ...
                                                @if($get_role)
                                                    @foreach ($get_role as $role)
                                                        <option value="{{ $role->id }}" @if(isset($_GET['role_id']) && $_GET['role_id'] == $role->id ) selected @endif @if(empty($get_member)) {{ old('role_id') == $role->id ? 'selected' : '' }} @else {{ (isset($get_member) && $get_member->role_id == $role->id) ? 'selected' : '' ; }} @endif>{{ $role->title }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                            @error('role_id')
                                    ...
                                    @if((isset($_GET['role_id']) && $_GET['role_id'] == 7) || (!empty($get_member->provider_id)) )
                                    <div class="col-md-6">
                                        <div class="mb-3">
...
function AddBank() {
    var role_id = $("#role_id").val();
        var leadRoute = "{{ route('member.create') }}";
