                                            ...
                                            <select class="form-control custom-select-icon @error('route_id') is-invalid @enderror" name="route_id" @if(!empty($assignroute)) disabled @endif>
                                                ...
                                                @if($allroute)
                                                    @foreach ($allroute as $routes)
                                                        <option value="{{ $routes->id }}" @if(empty($assignroute)) {{ old('route_id') == $routes->id ? 'selected' : '' }} @else {{ (isset($assignroute) && $assignroute->route_id == $routes->id) ? 'selected' : '' ; }} @endif>{{ $routes->route }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                            @error('route_id')
                                ...
                                <tbody>
                                    ...
                                    <tr class="odd gradeX">
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('routeassign.view', $assign->id) }}" class="text-success me-2">
                                                <i class="fa fa-eye"></i>
                                            </a>
                                            {{-- <a href="{{ route('routeassign.edit', $assign->id) }}" class="text-primary me-2">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            <form action="{{ route('routeassign.destroy', $assign->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
